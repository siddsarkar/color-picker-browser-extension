export default class ChromeStorage {
  ...
  private readonly colorModeStorageKey = 'colorMode'
  ...
  async getColorMode(): Promise<string> {
    ...
    try {
      const result = await this.storage.get({
        [this.colorModeStorageKey]: ''
      })

      mode = result[this.colorModeStorageKey]
    } catch (error) {
      console.error('Error getting color mode from storage', error)
    }

    return mode
  }

  async setColorMode(mode: string): Promise<void> {
    try {
      await this.storage.set({ [this.colorModeStorageKey]: mode })
    } catch (error) {
      console.error('Error setting color mode to storage', error)
    }
  }
}
